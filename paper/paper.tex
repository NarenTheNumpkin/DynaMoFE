...
\end{align}
where $r_m^* = \ell_m - \frac{1}{M}\sum_{j=1}^M \ell_j$ is the centered expert relative loss, $\beta_1 = 0.5$, $\beta_2 = 1.0$, and $\lambda_\delta = 0.01$. The pairwise ranking loss directly teaches expert preference ordering:
\begin{equation}
    \mathcal{L}_{\text{rank}} = \frac{1}{\binom{M}{2}}\sum_{i < j} \log\left(1 + \exp\left[-\operatorname{sign}(\ell_j - \ell_i)(\hat{R}_j - \hat{R}_i)\right]\right),
\end{equation}
ensuring that whenever expert $i$ achieves lower loss than expert $j$ on sample $X$, its predicted risk satisfies $\hat{R}_i < \hat{R}_j$, directly maximizing routing correctness without scale distortions from outlier losses.
