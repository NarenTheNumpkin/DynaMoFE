...
\begin{align}
    ...
    \hat{\mathbf{R}}(\mathbf{d}) &= W_2 \mathbf{h} + \mathbf{b}_2 \in \mathbb{R}^M, \\
...
\end{align}
where $H(\mathbf{w}) = -\sum_m w_m \log w_m$ measures routing entropy. When routing uncertainty is high, $\alpha \to 0$ safely falls back to the static prior; under pronounced degradation divergence, $\alpha \to 1$ enables sharp expert reallocation.
...
The router parameters $\theta$ are trained using direct Huber risk supervision coupled with fused margin cross-entropy:
\begin{equation}
    \mathcal{L}(\theta) = \mathcal{L}_{\text{BCE}}(m_{\dynamofe}(X), y) + \frac{\beta}{M}\sum_{m=1}^M \operatorname{Huber}(\hat{R}_m(\mathbf{d}), r_m^*),
\end{equation}
where $r_m^* = \ell_m - \frac{1}{M}\sum_{j=1}^M \ell_j$ represents the relative sample-level loss of expert $m$, and $\beta = 1.0$. Unlike unconstrained gating or heavy diversity penalties ($\lambda_{\text{div}}=0$), this reliability-supervised objective directly trains the router to predict which experts will fail under specific physical transmission signatures.
...
\textbf{$\text{DynaMoFE}_{\text{adaptive}}$ (OOF Gating)} & 99.33 & 98.02 & 96.23 & 93.07 & 94.57 & 95.41 & 96.79 & 95.68 & 93.07 & 92.6\% \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{itemize}
    \item \textbf{$\text{DynaMoFE}_{\text{adaptive}}$ vs.\ Baselines:} Evaluated out-of-fold, the learned router achieves strictly positive, statistically significant confidence intervals across all environments against external baselines ($p < 0.05$). Against \fcg{}: on canonical clean video, $\Delta = +0.68$ \pp{} ($95\%$ CI: $[+0.19, +1.16]$); under H.264 CRF 35, $\Delta = +2.82$ \pp{} ($95\%$ CI: $[+1.18, +4.39]$); under composite Resize$\rightarrow$H.264, $\Delta = +3.67$ \pp{} ($95\%$ CI: $[+2.19, +5.22]$); and under H.264$\rightarrow$Resize, $\Delta = +3.23$ \pp{} ($95\%$ CI: $[+1.88, +4.61]$). Against \tall{}: canonical $\Delta = +0.95$ \pp{} ($95\%$ CI: $[+0.18, +2.04]$); H.264 $\Delta = +16.67$ \pp{} ($95\%$ CI: $[+13.51, +19.93]$).
    \item \textbf{Adaptive vs.\ Static Prior:} The difference between $\text{DynaMoFE}_{\text{adaptive}}$ and $\text{DynaMoFE}_{\text{static}}$ is tightly bounded within $\sim 0.07$ \pp{} (Clean: $-0.01$ \pp{}, Sealed Mean: $-0.07$ \pp{}, H.264: $-0.10$ \pp{}, all $95\%$ CIs span zero).
\end{itemize}

\subsection{Leave-One-Degradation-Out (Zero-Shot Generalization)}
To demonstrate that physical degradation routing generalizes beyond memorized codec categories, we conducted leave-one-degradation-out experiments: training the router on six environments and testing on a held-out codec never observed during training. On held-out H.265 ($\text{CRF}=32$), \dynamofe{} achieves \textbf{94.59\%} \auc{} (\textbf{+3.04 \pp{}} over standalone \fcg{} 91.55\%). On held-out WebP ($Q=50$), it achieves \textbf{96.29\%} (\textbf{+1.55 \pp{}} over \fcg{} 94.74\%). On held-out JPEG ($Q=40$), it achieves \textbf{98.08\%} (\textbf{+1.12 \pp{}} over \fcg{} 96.96\%). In every case, zero-shot physical degradation routing matches or exceeds the fixed static baseline without requiring knowledge of the codec identity.

\section{Conclusion}

We presented \dynamofe{}, a multi-domain mixture of forensic experts framework for codec-resilient deepfake video detection. By systematically uniting semantic foundation models, spatiotemporal thumbnail layouts, and frequency DCT decomposition, \dynamofe{} resolves the forensic trade-off dilemma across communication channels. In a matched benchmark across 700 \ffpp{} videos and six sealed codec environments, forensic synergy achieves a new state of the art (95.75\% sealed mean \auc{} in static fusion; 95.68\% in adaptive gating), outperforming all individual baselines with verified bootstrap significance. Furthermore, leave-one-out experiments prove that routing conditioned on physical degradation signatures generalizes to previously unseen transmission channels.
